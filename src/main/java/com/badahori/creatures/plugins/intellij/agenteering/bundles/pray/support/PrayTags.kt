...
object PrayTags {
    ...
    private val officialTagsLowercase = officialTags.map { it.toLowerCase() to it }.toMap()

    private val eggTagLiteralsLowercased = eggTagLiterals.keys.map { it.toLowerCase() to it }.toMap()
    ...
    fun isAutogenerated(tag: String, usesRSCRCommand: Boolean): Boolean {
        if (AUTOGENERATED_TAGS.matches(tag.trim()))
            return true
        return tag.toLowerCase() == "remove script" && usesRSCRCommand
    }
    ...
    fun getCorrectedCase(tag: String): String? {
        ...
        return officialTagsLowercase[tag.toLowerCase()] ?: eggTagLiteralsLowercased[tag.toLowerCase()]
    }
