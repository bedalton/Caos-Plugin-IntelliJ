...
class CaosScriptNamedConstReference(element: CaosScriptNamedConstant) : PsiPolyVariantReferenceBase<CaosScriptNamedConstant>(element, TextRange.create(1, element.textLength)) {
    ...
    private val isDeclaration by lazy {
        myElement.hasParentOfType(com.badahori.creatures.plugins.intellij.agenteering.caos.psi.api.CaosScriptConstantAssignment::class.java)
    }
    ...
    override fun isReferenceTo(element: PsiElement): Boolean {
        ...
        if (element.name != name)
            return false
        if (element.hasParentOfType(com.badahori.creatures.plugins.intellij.agenteering.caos.psi.api.CaosScriptConstantAssignment::class.java) && isDeclaration) {
            return false
        }
        if (!element.hasParentOfType(com.badahori.creatures.plugins.intellij.agenteering.caos.psi.api.CaosScriptConstantAssignment::class.java) && !isDeclaration) {
            return false
